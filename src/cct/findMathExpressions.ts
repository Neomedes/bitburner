...
export async function main(ns: NS) {
  const contracts = get_unsolved_contracts(ns, ns.enums.CodingContractName.FindAllValidMathExpressions)
...
async function solve(ns: NS, contract: MyContract) {
  ...
  const [data, target] = contract.data as [string, number]
   ...
   */
  async function rollingTest(correctEquations: string[], equation: string, original: string, target: number, index: number, equationResult: number, lastAdd: number) {
