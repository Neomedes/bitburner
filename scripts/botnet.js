import { error_t, warning_t, exec_script, OutputTable } from "lib/functions.js"
import { MyServer } from "lib/servers.js"
import { get_updated_server_list } from "util/update_data.js"
import { read_keep_ram_file, get_keep_ram } from "lib/keep_ram.js"
 ...
 */
function get_matching_servers(search_term, all_servers) {
 ...
 */
function print_single_server(ns, server, verbose) {
  if (verbose) {
    ns.tprintf("%s: Host gefunden: %s", ns.getScriptName(), server.host) // simple connect using this script
    ns.tprintf("%-10s: %s", "Root", server.nuked ? "Ja" : "Nein")
    ns.tprintf("%-10s: %s", "Backdoor", server.backdoor ? "Ja" : "Nein")
    if (server.max_ram > 0)
      ns.tprintf("%-10s: %s", "RAM", ns.formatRam(server.ram_used) + " of " + ns.formatRam(server.max_ram))
    if (server.max_money > 0)
      ns.tprintf("%-10s: %s", "Geld", ns.formatNumber(server.current_money) + " of " + ns.formatNumber(server.max_money))
    ns.tprintf("%-10s: %s", "Sicherheit", ns.formatNumber(server.current_security, 1, 1000000) + `; Min: ${server.min_security}`)
 ...
 */
function find_server(ns, search_term, servers, silent) {
 ...
 * @param {boolean} silent No output when no servers are found.
 */
function connect_server(ns, search_term, servers, silent) {
  const matching_servers = get_matching_servers(search_term, servers)
  if (matching_servers.length === 1) {
 ...
 * @param {boolean} silent No output when no servers are found.
 */
async function open_backdoor(ns, search_term, servers, silent) {
  const matching_servers = get_matching_servers(search_term, servers)
  if (matching_servers.length === 1) {
 ...
 * @param {boolean} silent No output when no servers are found.
 */
async function open_backdoors(ns, search_terms, servers, silent) {
 ...
 * @param {MyServer[]} all_servers All servers available.
 * @param {MyServer[]} paddings Padding for subsequent lines
 */
function print_server_tree(ns, server, all_servers, paddings = []) {
 ...
 */
function print_server_stats(ns, servers, show_all, specific_hosts) {
  ...
  function compute_score(s) {
    return (player_hack_level < (s.hack_needed * 2)) ? -1 : (s.max_money / s.min_security)
  }
  ...
  const output_servers = (() => {
    ...
    return servers.filter(s => s.host !== "home" && s.nuked && s.max_money > 0 && s.hack_needed <= player_hack_level)
  })().map(s => {
    return {
      ...s,
 ...
 * @param {string} script
 * @param {string[]} hosts
 * @param {any[]} scripts_args
 * @param {boolean} silent
 */
function run_script(ns, servers, script, hosts, scripts_args, silent) {
 ...
 * @param {boolean} silent
 */
function clear_scripts(ns, servers, include_hosts, exclude_hosts, silent) {
...
export async function main(ns) {
  ...
  if (OPTS.reload) {
    ...
    find_server(ns, OPTS.find, servers, OPTS.silent)
  } else if (OPTS.connect !== "") {
    connect_server(ns, OPTS.connect, servers, OPTS.silent)
  } else if (OPTS.backdoor) {
    await open_backdoors(ns, OPTS.h, servers, OPTS.silent)
  } else if (OPTS.run !== "") {
    run_script(ns, servers, OPTS.run, OPTS.h, OPTS["_"], OPTS.silent)
  } else if (OPTS.clear) {
    clear_scripts(ns, servers, OPTS.h, OPTS.n, OPTS.silent)
  } else {
    print_server_stats(ns, servers, OPTS.all, OPTS.h)
  }
