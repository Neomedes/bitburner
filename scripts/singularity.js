import { warning_t, finished, is_empty_str, share_entries, intersect } from "lib/functions.js"
import { MyAugment } from "lib/sing_augs.js"
 ...
 */
function get_changed_stats(ns, augment) {
  const changed_stats = []
  for (let k in augment.stats) { if (augment.stats[k] !== 1) { changed_stats.push(ns.sprintf("%s: %+3d%%", k, (augment.stats[k] - 1) * 100)) } }
  return changed_stats.join(", ")
}

/** @param {string[]} possible_factions @param {string} filter @return {string[]} */
function get_filtered_factions(possible_factions, filter) {
  if (is_empty(filter)) return possible_factions
...
export async function main(ns) {
  ...
  const all_augments = await get_updated_augment_list(ns, OPTS.update || OPTS.pal)

  let done_something = false
  if (OPTS.facs) {
    done_something = true
    const factions = get_filtered_factions(Object.values(ns.enums.FactionName), OPTS.f)
    factions.sort()
  ...
  if (OPTS.augs) {
    ...
    const augmentations = all_augments
      ...
      .filter(a => is_empty_str(OPTS.a) || a.name.includes(OPTS.a))
    augmentations.sort()

    for (let augment of augmentations) {
      const stats = get_changed_stats(ns, augment)
      if (OPTS.s !== "" && !stats.toLowerCase().includes(OPTS.s.toLowerCase())) { continue }
  ...
  if (OPTS.pal) {
    ...
    // preferences
    const ignore_money = OPTS.all || OPTS.imon
    const ignore_reputation = OPTS.all || OPTS.irep
    const use_all_factions = OPTS.all || OPTS.ifac
    ...
    function get_max_reputation(factions) {
    ...
    /** @param {number[]} c @return {number} */
    function total_price(augs) {
    ...
    function rel_score(a) { return (a.score / a.price) }
    ...
    function line(nr, price, name, rep, score, factions) {
    ...
    if (pal.length < 1) {
      ...
      pal.forEach((p, i) => {
        ...
        line(i + 1, price, p.name, rep, score, factions)
    ...
    if (OPTS.all) {
      ns.tprintf(" ")
      ns.tprintf("Alle Augmentations (Preise jetzt):")
      ns.tprintf(" ")
      headline()
      separator()
      augmentations
        ...
        .forEach((p, i) => {
          try {
            const price = ns.formatNumber(p.price)
            const rep = ns.formatNumber(p.req_reputation)
            const score = ns.formatNumber(p.score)
            const factions = p.factions.filter(f => possible_factions.includes(f)).join(", ")
            line(i + 1, price, p.name, rep, score, factions)
            ...
            ns.tprintf("Fehler bei der Ausgabe von folgendem Augment:\n%s", JSON.stringify(p))
            ns.tprintf("%s", e.toString())
          }
        })
      if (augmentations.length % 3 !== 0) separator()
    }
    */
  }
